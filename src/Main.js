...
function Main() {
  ...
  const n = queryParameters.get('n') || 39.94966894827585; // Default latitude for New Jersey
  const w = queryParameters.get('w') || -74.79353732758622; // Default longitude for New Jersey
  const zoom = queryParameters.get('zoom') || 7; // Default zoom level for New Jersey
  ...
  const [mapBoxData, setMapBoxData] = useState(() => {
    ...
    return { center: { lon: -74.79353732758622, lat: 39.94966894827585 }, zoom: 7, style: 'open-street-map' }; // Default to New Jersey
  });
            ...
            data={[
              {
                ...
                hovertemplate: Object.values(data.counties).map((value) => {
                  return `<b>
                    ...
                    Number of Suicide Deaths: <span style="color:red;">${
                      value.suicideData.deaths === 0 ? 'less than 10' : value.suicideData.deaths
                    }</span><br>
                    No Religious Attendance: <span style="color:red;">${
